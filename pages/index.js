...
//   }, [query])
  ...
  useEffect(() => {
    fetch(`https://real-time-web-search.p.rapidapi.com/search?q=${query}&limit=100`, options)
      .then(response => response.json())
      .then(response => setData(response))
      .catch(err => console.error(err));
  }, [query])
  ...
  return (
    <div className="dark:bg-[#202124]">
      <Navbar handleSearchClick={handleSearchClick} inputRef={inputRef} handleCloseClick={handleCloseClick} />
      <div className="w-[100vw] flex justify-center my-5">
        <div className="grid grid-cols-2 gap-5 mx-auto">
          {data?.data?.map(item => {
            return(
              <Card {...item} key={item.position}/>
            )
          })}
        </div>
      </div>
